...
| `/books/:id` | 蔵書詳細 | 一般社員以上 | 貸出・返却、編集（管理社員） |
| `/books/new` | 蔵書追加 | 管理社員のみ | 蔵書登録 |
| `/books/:id/edit` | 蔵書編集 | 管理社員のみ | 蔵書更新・論理削除 |
| `/users` | 利用者一覧 | 一般社員以上 | 登録・編集へ遷移（対象は利用者のみ） |
| `/users/new` | 利用者登録 | 一般社員以上 | 氏名・メールのみ登録（ロールは `general_user` 固定。パスワードは API 側生成） |
...
管理社員向けの蔵書更新・削除は蔵書編集画面（`/books/:id/edit`）で行う。利用者の更新・論理削除は利用者編集画面（`/users/:id/edit`）で行う。削除専用ルートは設けない。
...
- 利用者登録（`/users/new`）は氏名・メールのみ入力します。初回パスワードは API が生成し Keycloak へ temporary で設定します（画面・レスポンスには出さない）。
- 削除専用ルートは設けない。削除ボタンは各編集画面に置く。
- 利用者の論理削除は `/users/:id/edit` 内の削除操作で行う。成功後は `/users` へ遷移する。貸出中がある場合は削除不可（`409`）として画面内でエラー表示する。
- 蔵書の論理削除（`books.deleted=true`）は `/books/:id/edit` 内の削除操作で行い、管理社員のみが操作できる。成功後は `/books` へ遷移する。業務衝突・API エラーは共通ボディ `{ error, message }` を画面に表示する（詳細は [openapi-notes.md](./openapi-notes.md)）。
