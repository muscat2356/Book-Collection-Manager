...
- 認証は Keycloak。未ログイン時は `/login` へ誘導します（Protected Route 前提。F-01）。
...
| パス | 画面 | 権限 | 主な操作 |
|------|------|------|----------|
| `/login` | ログイン | 認証は全員可 / SPA 利用は一般社員以上 | Keycloak ログイン。社員ロール無しは弾く |
...
- 未ログイン状態で保護ルートにアクセスした場合は `/login` へリダイレクトします。
